...
%            decoded = viterbiDecoding(message, trellis)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function decoded = viterbiDecoding(message, trellis)

    k = log2(trellis.numInputSymbols);  % Number of input bits
    n = log2(trellis.numOutputSymbols); % Number of output bits
    paths = -1*ones(trellis.numStates,length(message)/n,3);	% Contains the total metrics of each state at each transition, the transmitted symbol and its position
    ...
    for ii=1:trellis.numInputSymbols
       paths(trellis.nextStates(initial_state+1,ii)+1,cpt,1) = distanceMin(message(1:1+n-1),optiDe2Bi(trellis.outputs(initial_state+1,ii),n));  % Branch metric stored in the first dim of paths
       paths(trellis.nextStates(initial_state+1,ii)+1,cpt,2) = ii-1;    % Transmitted symbol stored in the second dim of paths
    end
    
    % Generalization
    for jj=n+1:n:length(message)
        ...
        for kk=1:trellis.numStates
           if (paths(kk,cpt-1,1) ~= -1)
               ...
               for ii=1:trellis.numInputSymbols
                   branchMetric = distanceMin(message(jj:jj+n-1),optiDe2Bi(trellis.outputs(kk,ii),n));  % Current branch metric
    ...
    for j=2:length(message)/n
        decoded = [decoded optiDe2Bi(paths(traceback_pos,j,2),k)];  % We append the transmitted symbols for each transition of the good path
